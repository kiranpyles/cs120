...
def sat_3_coloring(G):
    ...
    for v, n in enumerate(G.edges):
        for u in n:
            if u > v:
                solver.add_clause([-v * 3 - 1, -u * 3 - 1])
                solver.add_clause([-v * 3 - 2, -u * 3 - 2])
                solver.add_clause([-v * 3 - 3, -u * 3 - 3])
    ...
    if solution:
        for v in range(G.N):
            if solution[3 * v + 0 + 1] > 0:
                G.colors[v] = 0
            elif solution[3 * v + 1 + 1] > 0:
                G.colors[v] = 1
            else:
                G.colors[v] = 2
